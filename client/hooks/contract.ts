...
export const CONTRACT_ADDRESS =
  "CABPUAZMPXTJLFFMJMOPQPCAIXIQZDVVVXZSIZTUXOKECY5QHVUDTXRF";
...
// ============================================================
// Cloud Storage — Contract Methods
// ============================================================

/**
 * Upload/store data in the cloud.
 * Calls: upload(user: Address, key: Symbol, value: String)
 */
export async function uploadData(
  ...
  return callContract(
    "upload",
    [toScValAddress(caller), toScValSymbol(key), toScValString(value)],
    caller,
    true
  );
}
...
/**
 * Retrieve stored data (read-only).
 * Calls: get(user: Address, key: Symbol) -> String
 */
export async function getData(
  ...
  return readContract(
    "get",
    [toScValAddress(caller || "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWG2"), toScValSymbol(key)],
    caller
  );
}
...
/**
 * Delete stored data.
 * Calls: delete(user: Address, key: Symbol)
 */
export async function deleteData(
  ...
  return callContract(
    "delete",
    [toScValAddress(caller), toScValSymbol(key)],
    caller,
    true
  );
}
